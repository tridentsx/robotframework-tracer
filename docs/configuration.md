...
- **Type**: String
- **Default**: `rf`
- **Description**: Service name that appears in traces. Use `auto` to derive from suite name — each pabot worker will register as a separate service (e.g. `Jira`, `Grafana`, `Jenkins`).
- **Example**: `api-tests`, `ui-tests`, `auto`
